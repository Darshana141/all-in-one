...
$qry1 = "SELECT  * from aos_invoices where id = '".$odercartid."' and deleted = 0";
                $ans1 = $conn->query($qry1);
                if ($ans1->num_rows>0){

                    while ($cart_prod1 = $ans1->fetch_assoc()){
                    $datecreated = $cart_prod1['description'];
                    }
                }
?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
<!-- Mirrored from red-parts.php.themeforest.scompiler.ru/themes/red-ltr/cart.php by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 04 May 2020 20:25:55 GMT -->
...
<img src="backbutton.png"  class="block-header__title"  width="45px" style="margin-right: 45%;"></a>

        <h1 class="block-header__title" style="margin-right: 40%;font-size: 170%;margin-top: -7%;">Finalised and Sent Cart History <?php echo $datecreated ?></h1>
    </div>
</div>
</div>
            ...
            <table class="">
                <thead class="cart-table__head">
                    <tr class="cart-table__row" style="font-size: 15px;">
                        ...
                        <th class="cart-table__column cart-table__column--product" style="width: 70%;"><center>Product </center></th> 
...
$shipping = 0;
$total = 0;
$qry = "SELECT  * from aos_invoices where id = '".$odercartid."' and deleted = 0";
                $ans = $conn->query($qry);
                if ($ans->num_rows>0){

                    while ($cart_prod = $ans->fetch_assoc()){

    $sql = "SELECT DISTINCT * FROM aos_products_quotes where parent_id = '".$odercartid."'";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
     ...
     $sql1 = "SELECT DISTINCT * FROM aos_products,aos_products_cstm where id=id_c and id = '".$row["product_id"]."'";
...
<td class="cart-table__column cart-table__column--product"><b> 
    Our Code: <?php echo $row["part_number"]; ?></b>, 
    <?php echo $row1["name"]; ?></a>
, <?php echo $row1["vehicle_c"]; ?> - <?php echo $row1["model_c"]; ?> 
</td>
...
<?php 
 ...
 $totalamt = number_format((float)$cart_prod['total_amt'], 0, '', '');  
    }
}
...
₹<?php echo number_format((float)$row['product_unit_price'], 0, '', '');    ?></td>

<td class="cart-table__column cart-table__column--quantity" data-title="Quantity"> 
<?php echo number_format((float)$row['product_qty'], 0, '', '');  ?>
</td>

<td class="cart-table__column cart-table__column--total" data-title="Total">
₹<?php echo number_format((float)$row['product_total_price'], 0, '', '');    ?></td>
...
myPopup ('https://maxvisionparts.in/conform.php?grandtotal='+grandtotal, 'myWindow', 420, 220)
