...
width: 20px;
height: 15px;
"><?php echo $countordercart; ?></span>
<p class="newlogodescriptionresize2" style="
font-size: 12px;
margin-left: -35px;
">Cart&nbsp;<?php echo $countordercart; ?>&nbsp;item</p>
</span>
...
?>
    ...
    <li class="breadcrumb__item breadcrumb__item--parent"><a href="#" style="color: black; position: relative; top: 10px; left:0%; font-size: large;"> Recommended
    </a></li>
    </ol>
    </nav>
    </div>
...
$sqlcu = "SELECT * FROM contacts,contacts_cstm where id=id_c and  id = '".$rid."' and deleted = 0 ";
$resultcu = $conn->query($sqlcu);
$rowcu = $resultcu->fetch_assoc();


$sqlre = "SELECT * FROM `prospects` where department='".$rowcu['accode_c']."' order by last_name asc ";
$resultrec = $conn->query($sqlre);
...
$sql = "SELECT * FROM aos_products,aos_products_cstm where id=id_c and  part_number = '".$rowrec['last_name']."' and deleted = 0 ";
$result = $conn->query($sql);
...
{
?>
<a href="" target="_blank"  style="color: black;"><?php  echo $name = $row["name"];?> <?php $len = strlen($name); while ($len<50) { echo "&nbsp; "; $len++;} ?></a>
<?php 
}
?>
...
<td style="width: 50%;">&nbsp;Unit:<?php echo $row["unit_c"]; ?></td>
                    </tr>
                </table>
...
<div class="view-options__legend">Showing <?php echo $count;?> products</div>
    ...
    <!-- copyright -->
    <!-- copyright / end -->
    Copyright © 2020 <a href="https://maxvisionparts.in/">maxvisionparts.in </a>   All Rights Reserved.</a> 
...
{ 
  echo  '<a class="mobile-menu__indicator" href="cart.php">';
    ...
    <span class="indicator__counter"><?php echo $countordercart;?>  Item</span>
    </a>
