...
<div class="card-header">
    <h5><b>Finalised and Sent Cart History</b></h5>
</div>
    ...
    <div class="table-responsive-sm">
        <table>
            <thead>
                <tr>
                    <th><b>Cart No</b></th>
                    <th><b>Date & Time of cart FInalised</b></th> 
                 </tr>
            </thead>
            <tbody>
            <?php
...
$count = 1;
 $qry = "SELECT * from aos_invoices where billing_contact_id = '".$rid."' and deleted = 0  ORDER BY `aos_invoices`.`description` DESC"; //customer cart products
$ans = $conn->query($qry);
if ($ans->num_rows>0){
while ($cart_prod = $ans->fetch_assoc()){
if($cart_prod['description'] == "")
{
    $dateorder = '-';
}
else
{
    $dateorder =  $cart_prod['description'];
}
            ?> 
                <tr>
                    <td><a href="ordercart.php?id=<?php echo $cart_prod["id"]; ?>" target="_blank"><?php echo $count; ?></a></td>
                    <td><a href="ordercart.php?id=<?php echo $cart_prod["id"]; ?>" target="_blank"><?php echo $dateorder; ?></a></td>
                  </tr>
                <?php
            $count = $count+1;
            }
    ...
    $dateorder =  $cart_prod['description'];
            ...
            ?> 
                <tr>
                    <td><a href="ordercart.php?id=<?php echo $cart_prod["id"]; ?>" target="_blank"><?php echo $count; ?></a></td>
                    <td><a href="ordercart.php?id=<?php echo $cart_prod["id"]; ?>" target="_blank"><?php echo $dateorder; ?></a></td>
                    <td>₹ <?php echo number_format((float)$cart_prod['total_amt'], 0, '', '');?></td> 
                </tr>
                <?php
            $count = $count+1;
            }
...
<!-- copyright --> 
<!-- copyright / end --> 
    Copyright © 2020 <a href="#" >maxvision.com </a> All Rights Reserved.
